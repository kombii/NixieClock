...
u8 Display_Date[13] = {0x32,0x30,0x31,0x33,0x2f,0x31,0x30,0x2f,0x32,0x30,0x20,0x37,0x57};
					//日期显示缓存   格式  1998/05/14 7W

Time_Typedef TimeValue;	//时间数据指针
...
////////////////////////////////时间寄存器读或者写（0写1读）////////////////////////////////
//这个嘛。。。。需要外部定义好TimeValue结构体，调用直接写入内部值
...
{
	if(DS3231_Read_Byte(Address_control_status) & 0x80)  //晶振不干了
	{
		return 1;  //异常
	}
	else if(DS3231_Read_Byte(Address_control) & 0x80)  //或者 EOSC禁用
	{
		return 1;  //异常
	}
	else	return 0;  //正常
